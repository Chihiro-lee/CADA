...
/*@
  ...
  requires \valid(reg_r6);
  requires \valid(reg_sr);
  requires \valid(FCTL1);
  requires \valid(FCTL3);
  requires \valid(DFI_MAX);
  requires \valid(&DFI_MAX);
  requires \valid(DFI_MIN);
  requires \separated(reg_r4, reg_r5, reg_r6, reg_r8, reg_sr, FCTL3, FCTL1,DFI_MAX,DFI_MIN);
  assigns tmp_r4, *DFI_MAX, *reg_r4;
  assigns DFI_MAX, *reg_r6, *reg_sr, *FCTL1, *FCTL3;
  ensures DFI_MAX < DFI_MIN ==> DFI_MAX == DFI_MAX_Orig;
*/

void read_mov_fun(uint16_t* reg_r4, uint16_t* reg_r5, uint16_t* reg_r6, uint16_t* reg_sr) {
    *FCTL3 = 0xa500;
    *FCTL1 = 0xa540;
    tmp_r4 = *reg_r4;
    *DFI_MAX= *reg_r5;
