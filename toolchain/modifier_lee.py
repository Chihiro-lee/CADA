...
### 新增代码开始：定义读指令的正则表达式模式 ###
# 定义用于匹配所有 MOV 变体的正则表达式
read_valid_reg = r"(R(1[0-5]|[0-3]|[9])|PC|SR|SP)"  
read_source_registerPart_IndexMode = r"(R(1[0-5]|[0-3]|[9])|PC|SR|SP)" 
read_dest_registerPart_IndexMode = r"(R(1[0-5]|[0-3]|[9])|PC|SR|SP)" 
read_source_indirect = r"(@\s*({})\s*\+?)".format(read_valid_reg)  # @R12
read_source_indexed = r"(\s*(\d+)\s*\(\s*({})\s*\))".format(read_source_registerPart_IndexMode)  # 2(R1)
read_source_reg = r"({})".format(read_valid_reg)  # R1
read_source_modes = r"({}|{}|{})".format(read_source_indirect, read_source_indexed, read_source_reg)  # @Rn | n(Rn) | Rn
read_dest_indexed = r"(\s*(-|\+)?\d+\s*\(\s*({})\s*\))".format(read_dest_registerPart_IndexMode)  # 1(R14)

read_mov = re.compile(r"^\s*MOV\s+{}\s*,\s*({}|{})\s*$".format(read_source_modes, read_valid_reg, read_dest_indexed), re.I | re.M)
read_movw = re.compile(r"^\s*MOV.W\s+{}\s*,\s*({}|{})\s*$".format(read_source_modes, read_valid_reg, read_dest_indexed), re.I | re.M)
read_movb = re.compile(r"^\s*MOV\.B\s+{}\s*,\s*({}|{})\s*$".format(read_source_modes, read_valid_reg, read_dest_indexed), re.I | re.M)
read_movx = re.compile(r"^\s*MOVX\s+{}\s*,\s*({}|{})\s*$".format(read_source_modes, read_valid_reg, read_dest_indexed), re.I | re.M)
read_movxb = re.compile(r"^\s*MOVX\.B\s+{}\s*,\s*({}|{})\s*$".format(read_source_modes, read_valid_reg, read_dest_indexed), re.I | re.M)
read_mova = re.compile(r"^\s*MOVA\s+{}\s*,\s*({}|{})\s*$".format(read_source_modes, read_valid_reg, read_dest_indexed), re.I | re.M)
read_movxa = re.compile(r"^\s*MOVX\.A\s+{}\s*,\s*({}|{})\s*$".format(read_source_modes, read_valid_reg, read_dest_indexed), re.I | re.M)
### 修改代码结束 ###
...
read_mov_fun = "read_mov_fun"

secure_update= "receiveUpdate"

secure_send = "sendCount"

#hash_compute = "addressHash"
...
### 修改代码开始：替换 replaceRead 函数以支持 MOV Rx, Ry, MOV @Rx, Ry, MOV immediate(Rx), Ry ###
def replaceRead(match, mov_variant="MOV"): 
    logging.debug("Read instruction caught: {}".format(match.group(0)))
    source = match.group(1)  # 完整的源操作数 (@Rx, n(Rx), 或 Rx)
    dest_reg = match.group(14)  # 目标寄存器，如 R10
    #dest_indexed = match.group(9)  # 目标索引模式，如 1(R14)

    logging.debug("Pattern: {}, Source: {}".format(match.re.pattern, source))
    logging.debug("Groups: {}".format([match.group(i) for i in range(len(match.groups()) + 1)]))

    indirect = match.group(1)  # @R12
    indexed = match.group(1)   # 2(R1)
    reg_source = match.group(1)  # R1（新增分组）

    logging.debug("indirect: {}, indexed: {}, reg_source: {}".format(indirect, indexed, reg_source))

    if indirect:  # @Rx
        source_full = match.group(1).strip()  # @R1
        logging.debug("Indirect mode: source_full = {}".format(source_full))
    elif indexed:  # immediate(Rx)
        source_full = match.group(1).strip()  # 4(R1)
        logging.debug("Indexed mode: source_full = {}".format(source_full))
    elif reg_source:  # Rx
        source_full = match.group(1).strip()  # R1
        logging.debug("Register mode: source_full = {}".format(source_full))
    else:
        logging.error("No valid source mode matched for: {}".format(source))
        source_full = "; ERROR: Invalid source {}".format(source)

    new_instructions = Template(
        "\n\n; Old Instruction: $old\n"
        "\tMOV SR, $sr_pointer\n"
        "\tDINT\n"
        "\tNOP\n"
    ...
    return new_instructions.substitute(
        old=match.group(0),
        mov_variant=mov_variant,
        source=source_full,
        dest=dest_reg.strip() if dest_reg else match.group(10).strip(),
        sr_pointer=store_sr_pointer,
        src_pointer=store_src_pointer,
        dst_pointer=store_dst_pointer
    )
### 修改代码结束 ###
...
def execReplace(match,labelToCall,mode=0,offset=0):
    ...
    newInstructions = Template(
        ...
        "\tMOV SR, $srPointer\n"
        "\tDINT\n\tNOP\n"
        #we need to open it when we are doing local CFI check, otherwise we comment out this instruction
        #"\tMOVA PC, &address_pc\n"
        "$mov"
        "\t$jump #$safeBr\n"
        ";End safe sequence\n\n"
...
while line:    
    ...
    ### 新增代码开始：检查所有 MOV 变体的读指令 ###
    elif(match := read_mov.search(line)):
        newEntry = replaceRead(match, mov_variant="MOV")
        read_instructions += 1
    elif(match := read_movb.search(line)):
        newEntry = replaceRead(match, mov_variant="MOV.B")
        read_instructions += 1
    elif(match := read_movw.search(line)):
        newEntry = replaceRead(match, mov_variant="MOV.W")
        read_instructions += 1
    elif(match := read_movx.search(line)):
        newEntry = replaceRead(match, mov_variant="MOVX")
        read_instructions += 1
    elif(match := read_movxb.search(line)):
        newEntry = replaceRead(match, mov_variant="MOVX.B")
        read_instructions += 1
    elif(match := read_mova.search(line)):
        newEntry = replaceRead(match, mov_variant="MOVA")
        read_instructions += 1
    elif(match := read_movxa.search(line)):
        newEntry = replaceRead(match, mov_variant="MOVX.A")
        read_instructions += 1
    ### 新增代码结束 ###
...
### 新增代码开始：记录读指令计数 ###
logging.warning("\nTotal read instructions replaced:{}".format(read_instructions))
#logging.warning("\nTotal mov immediate instructions replaced:{}".format(mov_immediate_instructions))
