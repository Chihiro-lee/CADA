...
for f in sys.argv[2:]:
    inputName = f
    input = open(inputName, 'r')
    #print("Parsing file {} for illegal instructions".format(inputName))
    ...
    while line:
        if (RESERVED_REGISTER_CHECK):
            ...
            ### RESERVED REGISTER CONTROL ###
            if(match):
                #print("Reserved register used #{}: '{}'".format(cnt,match.group(0)))
        ...
        if(match := indirectPcAccess.search(line)):
            #print("Indirect access to PC #{}: '{}'".format(cnt,match.group(0)))
        ...
        ### POPM to PC ####
            ...
            #print("POPM to PC #{}: '{}'".format(cnt,match.group(0)))
        ...
        ######  BR + BRA checks #######
            ...
            if(match.group(2) != "0xff12" and match.group(2) != "0xff16" and match.group(2) != "0xff1a" and and not checkAddress(match.group(2),bottomInstructionMemory,topInstructionMemory,20)): #BR won't allow more than 16 bits           
        ...
        ######  CALL + CALLA checks #######
            ...
            if(not checkAddress(match.group(2),bottomInstructionMemory,topInstructionMemory,20)): #BR won't allow more than 16 bits
                #Either jump to forbidden area or read from bootloader
                print("Violating memory isolation with a CALL/CALLA #{}: '{}'".format(cnt,match.group(0)))
            ...
            if(not checkAddress(match.group(3),bottomInstructionMemory,topInstructionMemory,16)):
                #Either jump to forbidden area or read from bootloader
                print("Violating memory isolation with a MOV #{}: '{}'".format(cnt,match.group(0)))
            ...
            if(not checkAddress(match.group(2),bottomInstructionMemory,topInstructionMemory,8)):
                #Either jump to forbidden area or read from bootloader
                print("Violating memory isolation with a MOV.B #{}: '{}'".format(cnt,match.group(0)))
            ...
            if(not checkAddress(match.group(2),bottomInstructionMemory,topInstructionMemory,20)):
                #Either jump to forbidden area or read from bootloader
                print("Violating memory isolation with a MOVA #{}: '{}'".format(cnt,match.group(0)))
            ...
            if(not checkAddress(match.group(3),bottomInstructionMemory,topInstructionMemory,16)):
                #Either jump to forbidden area or read from bootloader
                print("Violating memory isolation with a MOVX #{}: '{}'".format(cnt,match.group(0)))
            ...
            if(not checkAddress(match.group(2),bottomInstructionMemory,topInstructionMemory,8)):
                #Either jump to forbidden area or read from bootloader
                print("Violating memory isolation with a MOVX.B #{}: '{}'".format(cnt,match.group(0)))
            ...
            if(not checkAddress(match.group(2),bottomInstructionMemory,topInstructionMemory,20)):
                #Either jump to forbidden area or read from bootloader
                print("Violating memory isolation with a MOVX.A #{}: '{}'".format(cnt,match.group(0)))
            ...
            print("Unlocking the FLASH controller  #{}: '{}'".format(cnt,match.group(0)))
